...
- **No credential may ever be a `buildConfigField`.** They compile to `public static final String`, so the literal is inlined before R8 runs and `strings` reads it straight out of the published APK. The SMTP mailer and the WhatsApp SSO secret were removed for exactly this reason; email verification now goes through Firebase, which holds its own credentials server-side. The one credential the app does carry, for phone verification, is read at run time from `app_config/whatsapp` and the gateway scopes it so narrowly that a copy of it can only send a code to somebody who has just asked for one.
